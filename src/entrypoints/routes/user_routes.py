...
from entrypoints.requests.find_user_request import FindUserRequest
...
async def find(request: Request):
    authorization = HandlerJwt().get_jwt_from_request(request)
...
async def delete(id: str):
    authorization = HandlerJwt().get_jwt_from_request(id)
    return DeleteUserController().delete(authorization)
